...
func TestAccountListEmpty(t *testing.T) {
	gpt := runGeth(t, "account", "list")
	gpt.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gpt := runGeth(t, "account", "list", "--datadir", datadir)
	defer gpt.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gpt := runGeth(t, "account", "new", "--lightkdf")
	defer gpt.ExpectExit()
	gpt.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gpt := runGeth(t, "account", "import", keyfile, "-password", passwordFile)
	defer gpt.ExpectExit()
	gpt.Expect(expected)
}
...
func TestWalletImport(t *testing.T) {
	gpt := runGeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gpt.ExpectExit()
	gpt.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gpt.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gpt.StderrText(), m) {
...
// https://github.com/PowerToken/ptchain_node/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gpt.SetTemplateFunc("keypath", func(file string) string {
