...
func (g *gethrpc) callRPC(result interface{}, method string, args ...interface{}) {
	if err := g.rpc.Call(&result, method, args...); err != nil {
		g.gpt.Fatalf("callRPC %v: %v", method, err)
	}
}

func (g *gethrpc) addPeer(peer *gethrpc) {
	g.gpt.Logf("%v.addPeer(%v)", g.name, peer.name)
	enode := peer.getNodeInfo().Enode
	peerCh := make(chan *p2p.PeerEvent)
	sub, err := g.rpc.Subscribe(context.Background(), "admin", peerCh, "peerEvents")
	if err != nil {
		g.gpt.Fatalf("subscribe %v: %v", g.name, err)
	}
	...
	select {
	case ev := <-peerCh:
		g.gpt.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type, ev.Peer)
	case err := <-sub.Err():
		g.gpt.Fatalf("%v sub error: %v", g.name, err)
	case <-timeout:
		g.gpt.Error("timeout adding peer after", dur)
	}
}
...
func (g *gethrpc) waitSynced() {
	...
	if ok && !syncing {
		g.gpt.Logf("%v already synced", g.name)
		return
	}
	...
	if err != nil {
		g.gpt.Fatalf("%v syncing: %v", g.name, err)
	}
	defer sub.Unsubscribe()
	timeout := time.After(4 * time.Second)
	select {
	case ev := <-ch:
		g.gpt.Log("'syncing' event", ev)
		syncing, ok := ev.(bool)
		if ok && !syncing {
			break
		}
		g.gpt.Log("Other 'syncing' event", ev)
	case err := <-sub.Err():
		g.gpt.Fatalf("%v notification: %v", g.name, err)
		break
	case <-timeout:
		g.gpt.Fatalf("%v timeout syncing", g.name)
		break
	}
}
...
func startGethWithIpc(t *testing.T, name string, args ...string) *gethrpc {
	...
	g.gpt = runGeth(t, args...)
	// wait before we can attach to it. TODO: probe for it properly
	time.Sleep(1 * time.Second)
	var err error
	ipcpath := filepath.Join(g.gpt.Datadir, "gpt.ipc")
